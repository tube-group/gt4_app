...
static bool loadConfig(int argc, char *argv[], AppConfig &app)
{
    ...
    std::string configFile = "../config/SprayWeight.ini";
    ...
    app.logCfg.log_console = config.GetBoolDefault("log_console", app.logCfg.log_console);
    ...
    app.logCfg.filename = config.GetStringDefault("filename", "log/SprayWeightStamp.log");
    ...
    app.daemonMode = config.GetBoolDefault("daemon", app.daemonMode);
    app.pidFile = config.GetStringDefault("pid_file", app.pidFile);
...
// ---- Redis连接 ----
static bool initRedis(SprayWeightContext &ctx)
{
    auto &config = CConfig::GetInstance();
    try
    {
...
// ---- PostgreSQL连接 ----
static bool initPostgreSQL(SprayWeightContext &ctx)
{
    auto &config = CConfig::GetInstance();
    try
    {
...
int main(int argc, char *argv[])
{
    ...
    ctx_weight.weightTcpHost = CConfig::GetInstance().GetStringDefault("ip_address", ctx_weight.weightTcpHost);
    ctx_weight.weightTcpPort = CConfig::GetInstance().GetIntDefault("weight_port", ctx_weight.weightTcpPort);
    ...
    // 通知工作线程退出，避免 join 阻塞。
    ctx_spray.running.store(false);
    ctx_weight.running.store(false);
