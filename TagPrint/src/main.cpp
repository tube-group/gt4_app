...
static bool loadConfig(int argc, char* argv[], AppConfig& app)
{
    ...
    std::string configFile = "../config/TagPrint.ini";
    ...
    app.logCfg.filename        = config.GetStringDefault("filename", "log/tagprint.log");
    ...
    app.pidFile    = config.GetStringDefault("pid_file", "/var/run/tagprint.pid");
...
// ---- Redis连接 ----
static bool initRedis(TagPrintContext& ctx)
{
    auto &config = CConfig::GetInstance();
    try {
...
// ---- gplat连接 ----
static bool initGplat(TagPrintContext& ctx)
{
    auto &config = CConfig::GetInstance();
    try {
...
// ---- PostgreSQL连接 ----
static bool initPostgreSQL(TagPrintContext& ctx)
{
    auto &config = CConfig::GetInstance();
    try {
