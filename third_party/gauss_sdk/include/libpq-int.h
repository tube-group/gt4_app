...
#define MAX_BATCH_ERRORS 20
#define BATCH_ERROR_STEP 8
...
typedef struct PGBatchErrorMsg {
    int offset;
    char *errcode;
    PQExpBufferData* batch_error_message;
} PGBatchErrorMsg;
...
struct pg_result {
    ...
    int err_batch_offset;      /* error line number on batch error mode */
...
#ifdef HAVE_CE
    PGresAttValue** original_tuples;
#endif
    int batch_error_hwm;     // high water mark
    int batch_error_count;   // current batch errors
    PGBatchErrorMsg *batch_result; // batch result array
    char* filename;          // for LOAD DATA local ... save file path
};
...
typedef enum {
    ...
    PGASYNC_LOAD_REMOTE, /* LOAD DATA local transfer in progress */
    PGASYNC_UNKNOWN_PACK /* Unknown package */
} PGAsyncStatusType;
...
typedef struct pg_conn_host
{
   ...
   bool real_connect;          /* Whether to perform a real connection */
} pg_conn_host;
...
typedef enum {
    ...
    SERVER_TYPE_CLUSTER_MAINNODE,     /* Primary node of the cluster */
    SERVER_TYPE_CLUSTER_PRIMARY,      /* Primary node of the primary cluster */
    SERVER_TYPE_CLUSTER_STANDBY       /* Standby node of the primary cluster */
} PGTargetServerType;
...
struct pg_conn {
    ...
    char* keepalives_count;    /* maximum number of TCP keepalive
                                * retransmits */
    char* tcp_user_timeout;    /* the amount of time that transmitted data may remain unacknowledged
                                * before the TCP connection is forcibly closed */
    char* tcp_syn_retries;     /* the maximum number of times initial SYNs for an active TCP
                                * connection attempt will be retransmitted */
    char* rw_timeout;          /* read-write timeout during idle connection.*/
    char* sslmode;             /* SSL mode (require,prefer,allow,disable) */
    char* sslcompression;      /* SSL compression (0 or 1) */
    char* sslcipher;           /* client cipher filename */
    bool sslgmtls;             /* SSL GM TLS switch (ture or false) */
    char* sslkey;              /* client key filename */
    char* sslcert;             /* client certificate filename */
    char* sslenccert;          /* client GM encryption certificate filename */
    char* sslenckey;           /* client GM encryption key filename */
    char* sslrootcert;         /* root certificate filename */
    char* sslcrl;              /* certificate revocation list filename */
    char* requirepeer;         /* required peer credentials for local sockets */
    unsigned char cipher_passwd[MAX_PASS_LEN + 1];

#ifdef ENABLE_MULTIPLE_NODES
    char *redistribution;       /* redistribution node role */
#endif

    char *umdk_enabled;         /* umdk_enabled */
    ...
    PGTernaryBool cluster_mainnode; /* Primary node of the cluster */
    PGTernaryBool cluster_primary;  /* Primary node of the primary cluster */
    PGTernaryBool cluster_standby;  /* Standby node of the primary cluster */
    char* driver_type;              /* Indicates the upper-layer driver type of libpq */
    bool enable_host_cache;         /* Enable hosts Cache */
    bool second_conn;               /* Real connection for second polling */

    // for batch error mode
    bool batch_error_mode;   // if we are in receiving batch errors, is true
    int batch_error_max;     // max batch size
    PGresult* result_batch;  // result for batch errors

#ifdef HAVE_CE
    SemtcGaussClientLogic* client_logic;
#endif

    bool force_no_truncation;   /* force param values to have no truncation in text format */
};
...
extern void pqResetBatchError(PGconn* conn, int max);
extern void pqClearBatchError(PGconn* conn);
extern int pqGetBatchError3(PGconn* conn);
extern void registeProcessMsgCallBack(RecvMsgProcessFunction callback);
...
extern int pqPutMsgBytes(const void* buf, size_t len, PGconn* conn);
...
#define check_gets_s(r) securec_check_c((r), "", "")
