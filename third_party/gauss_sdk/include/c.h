...
#define err_ecode __msvc_errcode
#include <crtdefs.h>
#undef err_ecode
#endif
...
/* do not use to represent MAX LSN, use MAX_XLOG_REC_PTR instead */
...
#define SubTransactionIdIsValid(subTransactionId) ((bool)((subTransactionId) != InvalidSubTransactionId))
...
#define IsValidCommandId(cid) ((cid) != InvalidCommandId)
...
#define ALT_SESSION_ID_LEN 16
typedef struct AltSessionId {
    unsigned char data[ALT_SESSION_ID_LEN];
} AltSessionId;

inline bool alt_session_id_is_valid(const AltSessionId &alt_session_id)
{
    return *(uint64 *)alt_session_id.data != 0LLU || *(uint64 *)(alt_session_id.data + sizeof(uint64)) != 0LLU;
}

#define InvalidAltSessionId (AltSessionId{0})

typedef uint32 AltLtxid;

inline bool alt_ltxid_is_valid(AltLtxid alt_ltxid)
{
    return alt_ltxid != 0;
}

#define InvalidLogicalTransactionId (0u)
...
/*
 * MmgrMemSetAligned is the same as MemSet except it omits the test to see if
 * "start" is word-aligned.  This is okay to use if the caller knows a-priori
 * that the pointer is suitably aligned (typically, because he just got it
 * from mmgr_alloc(), which always delivers a max-aligned pointer).
 */
#define MmgrMemSetAligned(start, val, len)                                                                           \
    do {                                                                                                         \
 ...
 * MmgrMemSetAligned.  Beware of multiple evaluations of the arguments when using
 * this approach.
 */
...
#define MemSetLoop(start, val, len)                                                  \
    do {                                                                             \
        long *_start64 = (long *)(start);                                            \
        long *_stop64 = (long *)((char *)_start64 + (Size)(len));                    \
        uint128 *_start = (uint128 *)(((uint64)_start64 & 0X08) + (uint64)_start64); \
        uint128 *_stop = (uint128 *)((uint64)(_stop64)&0XFFFFFFFFFFFFFFF0);          \
        if (len != 0) {                                                              \
            *_start64 = 0;                                                           \
            *(--_stop64) = 0;                                                        \
        }                                                                            \
...
#define LOWBIT 0x01
#define IS_LOWBIT_SET(ch) ((unsigned char)(ch) & LOWBIT)
#define STATUS_OK 0
...
#define FEATURE_NOT_PUBLIC_ERROR(x)                                                \
    do {                                                                           \
        ...
        if (!IsInitdb && !g_instance.attr.attr_common.support_extended_features && \
            ...
            ERR_PROC(GAUSS_25637, ERR_LEVEL_ERROR, (err_ecode(ERRCODE_FEATURE_NOT_SUPPORTED), \
                err_msg("Feature is not yet supported."),      \
                err_detail(x), err_cause("Feature is not yet support."), err_action_internal()));   \
    } while (0)
#endif /*PGXC*/
