...
typedef enum {
    ...
    CONNECTION_CHECK_CLUSTER_MAINNODE, /* Primary node of the cluster */
    CONNECTION_CHECK_PRIMARY_CLUSTER   /* Primary node or standby node of the primary cluster */
} ConnStatusType;
...
typedef enum {
    ...
    PGRES_LOAD_REMOTE,      /* LOAD DATA from remote to the server. */
    PGRES_UNKOWN_PACK      /* Unkown package */
} ExecStatusType;
...
typedef enum {
    STMT_DEFAULT = 0,
    STMT_BATCH_ERRORS    /* allowed batch errors for 'U' message */
} PQStmtAttr;
...
extern void PQconnectdbWithConnParam(const char* conninfo, PGconn** p_conn);
extern void PQconnectStartWithConnParam(const char* conninfo, PGconn** p_conn);
...
extern PGresult* PQexecPreparedBatchEx(PGconn* conn, const char* stmtName, int nParams, int nBatchCount,
    const char* const *paramValues, const int* paramLengths, const int* paramFormats, int resultFormat,
    PQStmtAttr attr, int attrValue);

extern int PQGetBatchErrorCount(PGresult* result);
extern int PQGetBatchErrorOffset(PGresult* result, int idx);
extern char* PQGetBatchErrorMessage(PGresult* result, int idx);
extern char* PQGetBatchErrorCode(PGresult* result, int idx);
extern void PGResetBatchErrorMode(PGresult* result);
...
extern int PQfnumberEx(const PGresult* res, const char* field_name, bool case_sensitive);
...
extern PGresult* getLoadResult(PGconn* conn, ExecStatusType exec_type);
extern char* PQGetLoadPath(PGconn *conn);
extern size_t PQGetCopyBinarySendSize(char *buf, size_t len, bool load_eof);

#ifdef HAVE_CE
/* TEE secure channels */
extern PGresult *PQtee_sec_channel(PGconn* conn, int stage, int index = 0);
#endif
extern bool PQenableTrustedDomain(PGconn *conn, bool send_or_clear);
