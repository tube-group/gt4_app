 ...
 * or arbitrary binary data.  All storage is allocated with mmgr_alloc().
 *
...
typedef struct InstanceNotExistReasonString {
    const char* level_string;
    int level_val;
} InstanceNotExistReasonString;

typedef struct LogLevelString {
    const char* level_string;
    int level_val;
} LogLevelString;

typedef struct InstanceDatanodeBuildReasonString {
    const char* reason_string;
    int reason_val;
} InstanceDatanodeBuildReasonString;

typedef struct InstanceTypeString {
    const char* type_string;
    int type_val;
} InstanceTypeString;

typedef struct GtmConString {
    const char* con_string;
    int con_val;
} GtmConString;

typedef struct InstanceCoordinatorActiveStatusString {
    const char* active_status_string;
    int active_status_val;
} InstanceCoordinatorActiveStatusString;

typedef struct InstanceDatanodeLockmodeString {
    const char* lockmode_string;
    uint32 lockmode_val;
} InstanceDatanodeLockmodeString;

typedef struct InstacneDatanodeRoleString {
    const char* role_string;
    uint32 role_val;
} InstacneDatanodeRoleString;

typedef struct InstacneDatanodeDbstateString {
    const char* dbstate_string;
    int dbstate_val;
} InstacneDatanodeDbstateString;

typedef struct InstacneDatanodeWalSendStateString {
    const char* wal_send_state_string;
    int wal_send_state_val;
} InstacneDatanodeWalSendStateString;

typedef struct InstacneDatanodeSyncStateString {
    const char* wal_sync_state_string;
    int wal_sync_state_val;
} InstacneDatanodeSyncStateString;

typedef struct ClusterStateString {
    const char* cluster_state_string;
    int cluster_state_val;
} ClusterStateString;

typedef struct ClusterMsgString {
    const char* cluster_msg_str;
    int cluster_msg_val;
} ClusterMsgString;

typedef struct ObsBackupStatusMapString {
    const char *obsStatusStr;
    int backupStatus;
} ObsBackupStatusMapString;

typedef struct ServerRoleString {
    int role_val;
    const char* role_string;
} ServerRoleString;
...
extern char** cm_read_file(const char* path);
extern void cm_free_file(char** lines);
...
extern const char* dcf_role_to_string(int role);
...
extern int32 obs_status_string_to_int(const char *statusStr);
...
extern int cm_execute_cmd(const char* command, struct timeval timeout);
extern int cm_init_masks(const int* ListenSocket, fd_set* rmask);
