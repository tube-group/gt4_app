...
void handleApiBundleDataEvent(CommL3Context &ctx, const char *value)
{
    ...
    mmwe_tqmtiwea14_t bundledata{}; // 值初始化，字符串为空，数值字段为零
    ...
    try
    {
        ...
        bundledata.mat_no = row["mat_no"].as<std::string>(); // 材料号(管捆号)
    ...
    writeb(ctx.gplatConn, "MMWE_TQMTIWEA14", &bundledata, sizeof(bundledata), &error);
    if (error != 0)
    {
