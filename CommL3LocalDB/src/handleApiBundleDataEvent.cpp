...
std::unordered_map<std::string, json> build_gspa13_values(const pqxx::result::reference &row, const std::string flag);
...
void handleApiBundleDataEvent(CommL3Context &ctx, const char *value)
{
    ...
    try
    {
        ...
        {
            result = ntx.exec(
                "SELECT * FROM api_bundle_data_del_t WHERE order_no = $1 AND item_no = $2 AND bundle_no = $3",
                pqxx::params{bundleEvent.order_no.to_string(), bundleEvent.item_no.to_string(), bundleEvent.bundle_no.to_string()});
        }

        if (result.empty())
        {
            spdlog::warn("No bundle record found for order_no={}, item_no={}, bundle_no={}",
        ...
        gspa13_values = build_gspa13_values(row, bundleEvent.flag.to_string()); // 构建 gspa13_values
    ...
    try
    {
        ...
        else
        {
            ...
            json rows = json::array();
            rows.push_back(l2sndl3.BuildRow(GSPA13::fields, gspa13_values));
            l2sndl3.SendRowsToRestIn("GSPA13", GSPA13::fields, rows, true, "发送HFW套管实绩删除到L3");
        }
    }
    catch (const std::exception &e)
    {
...
std::unordered_map<std::string, json> build_gspa13_values(const pqxx::result::reference &row, const std::string flag)
{
    return {
        ...
        {"MAT_THEORY_WT", std::round(row["theory_weight"].as<double>()) / 1000.0},
