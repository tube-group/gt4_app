...
namespace
{
	...
	void register_l3_request_api(httplib::Server &server, const std::string &route, CommL3Context &ctx)
	{
		server.Post(route, [&ctx](const httplib::Request &req, httplib::Response &res)
			...
			try
			{
				...
				// 2. 识别电文类型：优先读 ICMS_MESSAGE_NAME，降级使用字段匹配
				auto msg_name_it = payload.find("ICMS_MESSAGE_NAME");
				if (msg_name_it != payload.end() && msg_name_it->is_string())
				{
					const std::string icms_name = msg_name_it->get<std::string>();
					spdlog::info("ICMS_MESSAGE_NAME={}", icms_name);
					return;
				}
				else
				{
					spdlog::info("ICMS_MESSAGE_NAME not found");
				}
			}
			catch (const std::exception &ex)
			{
